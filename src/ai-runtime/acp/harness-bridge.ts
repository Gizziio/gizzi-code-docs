...
import { ACPMessage, ACPTool, ACPSession } from './schema.js';
import { ACPRegistry, acpRegistry } from './registry.js';
...
export class ACPHarnessBridge {
   ...
   */
  async processMessage(message: ACPMessage): Promise<ACPMessage | null> {
  ...
  private handleHandshake(message: ACPMessage): ACPMessage {
    return this.createResponse(message, {
      agentId: this.agentId,
      capabilities: this.capabilities,
      version: '1.0.0',
  ...
  private handleHeartbeat(message: ACPMessage): ACPMessage {
    // Update last seen
    const entry = this.registry.get(this.agentId);
    if (entry) {
  ...
  private createResponse(request: ACPMessage, data: unknown): ACPMessage {
    return {
