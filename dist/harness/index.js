...
export class AllternitHarness {
        ...
        switch (config.mode) {
            case 'byok':
                ...
                if (!config.byok.anthropic?.apiKey &&
                    ...
                    throw new HarnessError(HarnessErrorCode.CONFIG_INVALID, 'BYOK mode requires at least one provider API key (anthropic, openai, or google)');
                }
                break;
            case 'cloud':
    ...
    async *stream(request) {
        ...
        try {
            ...
            throw new HarnessError(HarnessErrorCode.UNKNOWN_ERROR, error instanceof Error ? error.message : 'Unknown error during streaming', error);
        }
    }
    /**
    ...
    async *streamFromBYOK(request) {
        ...
        switch (provider) {
            ...
            case 'google':
                ...
                break;
            default:
                throw new HarnessError(HarnessErrorCode.PROVIDER_NOT_FOUND, `Provider "${provider}" not supported in BYOK mode. Supported: anthropic, openai, google`);
        }
    }
    /**
    ...
    async *streamFromAnthropic(request) {
        ...
        // - Support tool use
        throw new HarnessError(HarnessErrorCode.API_ERROR, 'Anthropic streaming not yet implemented');
    }
    /**
