...
export class ACPRegistry {
    ...
    list(filter) {
        ...
        return entries.sort((a, b) => new Date(b.lastSeenAt).getTime() - new Date(a.lastSeenAt).getTime());
    }
    /**
     * Find agents by capability
     */
    findByCapability(capabilityName) {
        return this.list().filter(entry => entry.capabilities.some(cap => cap.name === capabilityName));
    }
    /**
    ...
    search(query) {
        ...
        return this.list().filter(entry => entry.name.toLowerCase().includes(lowerQuery) ||
     ...
     */
    createSession(agentId, permissions = ['read']) {
    ...
    validateMessage(message) {
        ...
        if (message.source.capability) {
            const hasCapability = entry.capabilities.some(cap => cap.name === message.source.capability);
            if (!hasCapability) {
    ...
    getStats() {
        ...
        return {
            ...
            totalCapabilities: entries.reduce((sum, e) => sum + e.capabilities.length, 0),
